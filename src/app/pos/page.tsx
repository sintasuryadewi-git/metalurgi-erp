...
export default function PosPage() {
    ...
    const analysisData = useMemo(() => {
        ...
        let totalNominal = 0; let totalQty = 0;
        ...
        // Menentukan sumber data utama berdasarkan Toggle (Lokal vs Cloud)
        const activeTrx = viewSource === 'local' ? allTransactions.filter(t => {
            const d = new Date(getTrxDate(t)); return d >= start && d <= end;
        }) : cldTrx;

        methods.forEach(m => {
            const trxs = activeTrx.filter(t => getTrxMethod(t) === m); // Sekarang patuh pada toggle
            ...
            if (c > 0 || cQty > 0) reconcileStats.push({ method: m, nominal: c, qty: cQty });
            totalNominal += c; totalQty += cQty;
        });
        ...
        const getMpTotal = (mName: string) => activeTrx.filter(t => getTrxMethod(t) === mName).reduce((a, b) => a + getTrxTotal(b), 0);
        ...
        return { reconcileStats, totalNominal, totalQty, rankedSKU, grandTotalQty, grandTotalAmount, estCommission, TotalNet };
    }, [allTransactions, cloudTransactions, dateRange, viewSource]);

    const runAutoSync = async (transaction: any) => {
        setIsSyncing(true);
        try {
            await apiClient('/api/pos/sync', {
                method: 'POST',
                body: JSON.stringify({
                    email: ownerEmail || 'rengganisdawet@gmail.com',
    ...
    const handleSyncShifts = async () => {
        ...
        setIsSyncing(true);
        try {
            const res = await apiClient('/api/pos/shift', {
                method: 'POST',
                body: JSON.stringify({
                    email: ownerEmail || 'rengganisdawet@gmail.com',
    ...
    const handleSyncToCloud = async () => {
        ...
        setIsSyncing(true);
        try {
            const res = await apiClient('/api/pos/sync', {
                method: 'POST',
                body: JSON.stringify({
                    email: ownerEmail || 'rengganisdawet@gmail.com',
                    transactions: trxs
    ...
    const handleCloseShift = () => {
        ...
        apiClient('/api/pos/shift', {
            method: 'POST',
            body: JSON.stringify({ email: ownerEmail || 'rengganisdawet@gmail.com', shiftData: closingData })
        }).catch(() => { });
    ...
    // =========================================================================
    // FUNGSI EMERGENCY BACKUP (Telah Disatukan untuk menghindari Error ts(2304))
    // =========================================================================
    const handleEmergencyBackup = () => {
        try {
            ...
            alert("✅ Data berhasil di-download langsung ke memori tablet!");
        } catch (error) {
            alert("❌ Gagal download manual: " + error);
        }
    };

    const pushToDriveEmergency = async () => {
        try {
            ...
            try {
                ...
                dbData = { databases: dbs.map(d => d.name) };
            } catch (e) { }

            const finalPayload = {
                timestamp: new Date().toISOString(),
                source: "Tablet Kasir Alya",
                localStorage: localData,
                indexedDB_Info: dbData
            };

            const res = await fetch('/api/emergency-drive', {
                method: 'POST',
            ...
            if (result.success) {
                alert("🚀 DATA TERKIRIM! File JSON sudah aman di Google Drive Ibu Sinta.");
            } else {
                throw new Error(result.error);
            }
        } catch (error) {
            alert("❌ Gagal Push ke Drive. Sistem akan otomatis mencoba download file ke tablet... Error: " + error);
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {basicMethods.map(m => (
                            <button
                                ...
                                className={`p-3 rounded-xl border flex flex-col items-center gap-2 transition-all ${paymentMethod === m ? 'border-blue-600 bg-blue-50 text-blue-700 shadow-sm' : 'border-slate-200 hover:bg-slate-50 text-slate-600'
                                    }`}
                            >
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {marketMethods.map(m => (
                            <button
                                key={m}
                                onClick={() => setPaymentMethod(m as any)}
                                className={`p-3 rounded-xl border flex flex-col items-center gap-2 transition-all ${paymentMethod === m ? 'border-orange-500 bg-orange-50 text-orange-700 shadow-sm' : 'border-slate-200 hover:bg-slate-50 text-slate-600'
                                    }`}
                            >
                                <Smartphone size={20} />
                        ...
                        <div className="flex justify-between items-center gap-3">
                            ...
                            {viewSource === 'local' && (
                                <>
                                    ...
                                    {/* ✨ TOMBOL DARURAT DRIVE ADA DI SINI */}
                                    <button onClick={pushToDriveEmergency} className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all shadow-sm bg-rose-500 hover:bg-rose-600 text-white animate-pulse" title="Push data ke Google Drive"><CloudUpload size={14} /> Darurat Drive</button>
                                    ...
                                    <table className="w-full text-xs text-left">
                                        <thead className="bg-slate-50 text-slate-500 border-b border-slate-100">
                                            <tr>
                                                ...
                                                {/* Header akan berubah otomatis menyesuaikan toggle */}
                                                <th className="p-2 text-right">Nominal ({viewSource === 'local' ? 'Lokal' : 'Cloud'})</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-50">
                                            {analysisData.reconcileStats.map((stat, idx) => (
                                                <tr key={idx} className="hover:bg-slate-50">
                                                    ...
                                                    {/* Menggunakan property .nominal */}
                                                    <td className="p-2 text-right font-mono text-blue-600">{fmtMoney(stat.nominal)}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                        <tfoot>
                                            <tr className="border-t-2 border-slate-200">
                                                ...
                                                {/* Menggunakan property .totalQty dan .totalNominal */}
                                                <th className="p-2 text-center font-bold text-slate-800">{analysisData.totalQty}</th>
                                                <th className="p-2 text-right font-bold text-blue-700">{fmtMoney(analysisData.totalNominal)}</th>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
                        ...
                        <div className="flex-1 flex flex-col gap-6">
                            ...
                            <div>
                                ...
                                <div className="space-y-3 bg-amber-50/50 p-4 rounded-xl border border-amber-100">
                                    ...
                                    <div className={`p-3 rounded-lg text-sm flex justify-between items-center shadow-inner ${endCashInput - (shiftData.startCash + allTransactions.filter(t => t.shiftId === shiftData.id && getTrxMethod(t) === 'Cash').reduce((acc, t) => acc + getTrxTotal(t), 0) - cashOutInput) === 0
